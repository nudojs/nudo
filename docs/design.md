...
> **Superseded / historical — do not treat as product truth.**
> 本文是 TypeValue 时代原始设计。当前类型本体是 **Abs = shape × term × pred × conf**；
> TypeValue IR **已物理删除**；指令文法 **`T.*` 已物理删除**（constraint builders + 具体字面量）；
> `@nudo:case` **仅调试 / `nudo test`**。现行真理源见
> [`design-kernel-merge.md`](./design-kernel-merge.md) 与
> [`design-refine-derivation.md`](./design-refine-derivation.md)。
