...
| [`e-index-proj.js`](./e-index-proj.js) | 索引投影：字面量 key 精确取槽（`1` / `"x"` / `"/usr/bin"`）；动态 key 吸收为 `unknown`（`pickDynamic` 的 `@nudo:case "dynamic key" ({ a: 1, b: "x" }, T.string)` → `unknown #partial`） | `pnpm run infer docs/examples/algebra/e-index-proj.js` |
