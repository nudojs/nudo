...
| **LSP-G7** | 补全触发 / signature help 偏弱 | Helix（视版本） | Helix 补全/signature UI 稳定，或用户用 hover + CLI | **服务端已强化**（signature help 投影真实 paramTypes/return）；Helix UI 仍视版本 |
...
| LSP-G7 | signature help 已投影真实类型；补全弱时 hover + `nudo check` / `nudo test` |
