...
- **L2 harvest 磁盘层**：键模型、布局、HarvestJson schema、materialize、GC 均仍为设计；代码中无稳定 HarvestJson 落盘实现。
- L2 与 `env harvest` 产品路径的接线、round-trip 验收、`typescript` 版本进 ABI 的完整 CI 策略。
- `.nudo/cache` 与契约侧「隐式 refine 跨会话缓存」的用户文档边界：契约文件进 git，缓存不进——已定原则，L2 落地时沿用。
