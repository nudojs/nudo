...
> **Superseded / historical.** 本文写于 `@nudo:returns` 时代。当前源码契约使用 `@nudo:refine` + `*.nudo.js` 模板，类型本体为 Abs（见 `docs/design-kernel-merge.md` 与 `docs/nudo-check.md`）。下文指令语法仅作历史参考。
...
为了提升语法的清晰度、健壮性和可维护性，我们将原有的行内注释（`// @`）优化为更结构化的块级注释（`/* @nudo:... */`）。

#### 3.3.1 设计原则

1.  **命名空间**：引入 `@nudo:` 命名空间，避免与其他文档工具（如 JSDoc）或注释指令冲突。
...
| 注释             | 作用                                                 | 示例                                                                    |
| ---------------- | ---------------------------------------------------- | ----------------------------------------------------------------------- |
| `/* @nudo:case */` | 提供一个具名执行样例，可包含内联数据或从外部文件导入。 | `/* @nudo:case "Success" { "id": 1 } */`                                 |
| `/* @nudo:mock */` | Mock 一个函数、模块或全局变量，可使用内联值或从文件导入。 | `/* @nudo:mock fs from "./mocks#mockFs" */`                              |
| `/* @nudo:returns */`| **(新增)** 验证最终推导出的类型是否符合预期。        | `/* @nudo:returns (type) => type.name === "string" */`                   |
| `/* @nudo:pure */` | 标记纯函数，以便进行缓存优化。                       | `/* @nudo:pure */`                                                      |
| `/* @nudo:sample */`| 指定循环操作的采样次数。                             | `/* @nudo:sample 100 */`                                                 |
| `/* @nudo:skip */`  | 跳过函数执行，直接采用其 TypeScript 类型注解。         | `/* @nudo:skip */`                                                      |
...
在这个例子中，我们使用 `@nudo:case` 来提供多个输入样例，并通过 `@nudo:returns` 来验证最终推导出的联合类型的正确性。

```typescript
/**
 * Processes an API response and extracts user data.
 *
 * @nudo:case "Successful response with users" {
 *   "users": [{ "id": 1, "name": "Alice" }],
 *   "total": 1
 * }
 * @nudo:case "Response with no users" {
 *   "users": [],
 *   "total": 0
 * }
 * @nudo:case "Invalid null data" null
 *
 * @nudo:returns (type) => {
...
```typescript
/**
 * @nudo:case "String validator" (v) => typeof v === "string"
 * @nudo:case "Number validator" (v) => typeof v === "number"
 */
