...
Primary CLI verbs for agents/CI: `nudo check` (signatures + gate), `nudo test` (case report), `nudo contract` (interfaces), `nudo export` (dts/guard/schema/standard), `nudo health` (drift), `nudo env harvest`. Observation is check/test/IDE hover.
