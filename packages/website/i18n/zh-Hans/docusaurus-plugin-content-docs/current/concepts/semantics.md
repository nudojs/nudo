...
- CI 上只分析**可信仓库**。
- 侧车契约（`*.nudo.js`）与 `@nudo:mock` / `@nudo:mock-module` 文件都是普通 JS，分析期同样会执行——同属一个信任边界。

这不是沙箱：Nudo **不**隔离求值进程。分析预算只防失控推断，不是安全边界。唯一事实源：仓库 `docs/design/kernel-merge.md` →「执行模型与信任边界」。
...
|---|---|---|
| Native bindings | `child_process.spawn`、原生 addon —— env 可有签名（ChildProcess pid/stdio/kill），无副作用模拟 | `@nudo:mock`，或把返回值当 opaque |
| 动态 `require` | 字面量 / 常量折叠子集已解析；计算说明符诚实 `unknown` + `nudo:builtin-unknown` | `@nudo:mock-module` / 静态 import |
| 流机器回调 | Node Transform 内部由运行时驱动，无调用点记录可 harvest；`transform`/`flush` 钩子形参已可 refine | mock 流工厂；不要期望内部回调被推断 |
