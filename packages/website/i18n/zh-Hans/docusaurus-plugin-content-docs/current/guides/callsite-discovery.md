...
匹配到的记录会作为合成的 `call@L` case 注入 `analyzeFile`，其中 `L` 是该调用在使用方文件中的行号。完全没有收到任何记录的函数仍会得到一个参数为 **`any`** 的 `entry@L` case，以便输出其签名——这些函数会被标记为仅入口（entry-only）。真 `unknown` 表示推导失败，不是「无调用点」。
