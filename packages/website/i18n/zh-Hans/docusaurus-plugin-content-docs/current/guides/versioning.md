...
| `@nudojs/service` | **5.x**（5.0.0-beta.1） | SemVer：破坏性 → major |
| `nudojs` | **1.x**（1.0.0-beta.3） | SemVer：破坏性 → major |
| `@nudojs/parser` | **1.x**（1.1.0-beta.0） | SemVer：破坏性 → major |
| `@nudojs/lsp` | **2.x**（2.0.0-beta.1） | SemVer：破坏性 → major。冻结清单：`packages/lsp/PUBLIC_API.md` |
| `@nudojs/env` / `@nudojs/harvester` | 0.x（0.4.2-beta.0 / 1.0.0-beta.1） | minor 可能破坏；为 IDE/CI 分析稳定可锁 minor。手写 env 在重叠模块/导出上 wins（`mergeHarvestUnderEnv`） |
| `vite-plugin-nudo` | 0.x（0.4.3-beta.1） | minor 可能破坏 |
...
| `@nudojs/harvester` | 1.0.0-beta.1（pre-1.0） | workspace / `~1.0.0-beta.1` | Harvest 是**旁路信道**，不是类型系统真理源。预算默认：`maxFiles=12`、`maxMs=2500`，`NUDO_HARVEST_NODE=off` 显式关闭。 |
<!-- NUDO-ECOSYSTEM:END -->
