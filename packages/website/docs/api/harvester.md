...
Results are cached in-process (key: package root + `package.json` mtime/size + budgets). Call `clearNodeHarvestCache()` after `@types/node` changes in watch/test paths. **Handwritten `@nudojs/env` wins over harvest** on overlapping module keys and export names — analysis injects via `mergeHarvestUnderEnv` in `@nudojs/service` (harvest only fills missing slots). Do not treat harvest output as the type-system source of truth.
