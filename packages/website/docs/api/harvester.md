...
| **Types package** — `@types/*` installed or the package ships `.d.ts` | Declarations are **harvested** (`harvestPackage` / `harvestNodeTypes` / `bareSpecToAbsModules` → env modules) | `nudo harvest`, analysis injection via module-graph harvest, handwritten `@nudojs/env` stays primary for builtins |
