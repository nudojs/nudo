...
`--emit` projects the observed call-site domain onto a sidecar `@generated` segment — a three-line block (`@generated` header, `// source: <file>:<fn>`, export line). Domain roots (exports with no in-file call sites) need `--callsites <paths...>` to feed usage-site evidence:
