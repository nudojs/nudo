...
- **Directive check**: Files without Nudo directives (`@nudo:case`, `@nudo:mock`, `@nudo:pure`, `@nudo:skip`, `@nudo:sample`, `@nudo:refine`, `@nudo:import`, `@nudo:env`, `@nudo:mock-module`, `@nudo:as`, `@nudo:replace`) are skipped. No analysis is run for them.
- **Analysis**: For matching files with directives, the plugin uses `analyzeFileAsync` from `@nudojs/service` to run type inference.
