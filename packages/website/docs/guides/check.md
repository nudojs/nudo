...
sidebar_position: 8
slug: /guides/check
description: nudo check — constraint, assign, and arg-structure gate on Abs (type-as-computation).
---

# nudo check

`nudo check` is Nudo's **gate on Abs** (type-as-computation). It reports whether call-site arguments satisfy preconditions, whether assignments and arguments are structurally compatible, and emits a **Nudo-native** report — not a TypeScript diagnostic in disguise.
...
| Code | Meaning |
|------|---------|
| `nudo:constraint-violated` | Call-site argument ⊭ precondition (`x > 0`) |
...
It does **not** replace `tsc` for full structural completeness. It does what tsc cannot on unannotated JS: **numeric constraints derived from the program**, plus **Abs leq** for assignments and call arguments.
